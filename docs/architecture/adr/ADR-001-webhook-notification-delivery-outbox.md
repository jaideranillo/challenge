...
| **Worst-case total** | **~7.8s** | ~2.2s margin under the 10s `VisibilityTimeout` |
...
5. **Sign and POST** (HMAC over body + timestamp, computed here per the OWASP A02/A04 row — see below) on the virtual thread, with the per-attempt timeouts from §4's budget table.
...
**Ordering (steps 6 then 7) is not incidental.** Deleting the SQS message before the attempt/status write commits would lose the attempt record entirely and strand the row in `PROCESSING` with no due-query re-entry until the 60s staleness reclaim (§6.1) — a self-inflicted version of the crash case that reclaim already exists to cover. Committing the DB write first and deleting after means the worst case on a crash between steps 6 and 7 is the message reappearing once `VisibilityTimeout` (10s, §10.1) elapses: harmless, because the conditional claim in step 1 already absorbs it (zero rows affected, delete, done) at a cost of one receive out of three. This is the same asymmetry §0 states generally ("a message lost in SQS is recoverable; a row lost in PostgreSQL is not") applied to this specific ordering decision.
...
These are the whiteboard's numbers (`docs/challenge/proposal/Fase_2/Configs_High_Level.png`), adopted as final, with the dependent mechanisms adjusted so they hold together:

| Setting | Value | Source |
| --- | --- | --- |
| `VisibilityTimeout` | **10s** | whiteboard |
...
Both numbers are tighter than what an earlier draft of this ADR proposed (`VisibilityTimeout` was never stated at all; `maxReceiveCount` was 50). Adopting them required two changes elsewhere, both of which make the design simpler rather than more complex:
...
**2. Per-attempt timeouts are trimmed to fit under 10s.** A `VisibilityTimeout` shorter than the worst-case in-flight time means routine redelivery, which both costs receives and produces duplicate-claim churn. The pre-existing budget (2s bulkhead acquire + 2s connect + 5s read + DB writes) summed to ~9.3-10s — at or over the limit with no margin. The bulkhead acquire timeout is therefore cut from 2s to **500ms** (§10.3); connect (2s) and read (5s) are unchanged, since those are real network budgets against arbitrary client endpoints and shrinking them would change delivery semantics, not just scheduling. The resulting worst case is ~7.8s (§4's budget table), leaving ~2.2s of margin — about 22%.

**The margin is stated plainly: it is adequate, not generous.** A GC pause, a slow `deliveries` write, or DNS resolution at the top of the connect budget can still push a single attempt past 10s. That case is *safe but not free*: the message reappears, the second worker's conditional claim affects zero rows (§6.2 steps 1-2), and it deletes — no double POST, because the state-guarded claim, not the visibility timeout, is what prevents duplicate sends. The cost is one receive out of three. Sustained occurrences would show up as a rise in zero-row claims, which is worth a dashboard line (§8) as the leading indicator that 10s needs revisiting.
...
- Acquisition uses a **500ms** timeout (trimmed from an earlier 2s proposal to fit the 10s `VisibilityTimeout` budget, §10.1, §4). Failing fast costs nothing here, because deferral is now cheap and explicit: waiting 2s for a local semaphore only consumed in-flight budget that the network legs need more.
...
- **Q1 — Queue technology (resolved: Amazon SQS).** Confirmed by the user: the delivery queue is AWS SQS Standard with long polling and a redrive policy to a DLQ (§0.1, §10.1), and the two whiteboard numbers in §10.1 (`VisibilityTimeout`, `maxReceiveCount`) are SQS settings, not placeholders. The rest of the design stays queue-agnostic anyway: SQS is load-bearing in exactly one place, the `NotificationQueuePort` implementation, and the design degrades to pure relay polling with no correctness change if the queue is removed entirely (§0, §6.1). Nothing downstream of this entry is blocked on it.
