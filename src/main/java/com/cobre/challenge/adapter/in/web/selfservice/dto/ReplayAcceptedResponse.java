 ...
 * original {@code DEAD} row and its history are untouched.
 */
public record ReplayAcceptedResponse(UUID deliveryId, DeliveryStatus status) {
    ...
    public static ReplayAcceptedResponse from(Accepted accepted) {
        return new ReplayAcceptedResponse(accepted.newDeliveryId(), accepted.status());
    }
}
