 ...
 * {@link TenantId} parameter and must use it as a predicate, never as a post-hoc filter
...
public interface DeliveryQueryRepositoryPort {
     ...
     *
     * <p>{@code tenant} is a query predicate, not a post-hoc filter. Returns
     * {@link Optional#empty()} when no row matches or when the row belongs to a different tenant
     * (ADR-007 §5.2, A01/IDOR).
     */
    Optional<Delivery> findById(UUID deliveryId, TenantId tenant);

    /**
     * Keyset-paginated delivery list for the self-service API (ADR-005 §1).
     *
     * <p>{@code tenant} is mandatory and is a query predicate. {@code eventCreatedFrom} and
     ...
     */
    DeliveryPage findPage(TenantId tenant, DeliveryPageQuery query, int limit);
}
